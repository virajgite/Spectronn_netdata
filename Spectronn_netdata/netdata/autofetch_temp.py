...
if os.path.exists("/var/www/coursewiki/search/static/netdata_systemload_service.json"):
    append_write = 'r' # append if already exists
else:
    append_write = 'w' # make a new file if not

f=open("/var/www/coursewiki/search/static/netdata_systemload_service.json",append_write)
if (append_write=='w'):
	j=json.dumps(j)
	f.write(j)
else:
	...
	d=open("/var/www/coursewiki/search/static/netdata_systemload_service.json","w")
	j=json.dumps(j)
	d.write(j)
	d.close()
